...
def resface12(images, keep_probability,
    ...
    with tf.variable_scope('Conv0'):
        net = resface_pre(images, 64 // 2, scope='Conv0_pre')  # 48x48
    with tf.variable_scope('Conv1'):
        net = resface_pre(net, 64 // 2, scope='Conv1_pre')  # 24x24
        net = slim.repeat(net, 1, resface_block, 64 // 2, scope='Conv1')  # 2
    end_points[ 'Conv1' ] = net
    with tf.variable_scope('Conv2'):
        net = resface_pre(net, 128 // 2, scope='Conv2_pre')  # 12x12
        net = slim.repeat(net, 1, resface_block, 128 // 2, scope='Conv2')  # 2
    end_points[ 'Conv2' ] = net
    with tf.variable_scope('Conv3'):
        net = resface_pre(net, 256 // 2, scope='Conv3_pre')  # 6x6
        net = slim.repeat(net, 1, resface_block, 256 // 2, scope='Conv3')  # 2
    end_points[ 'Conv3' ] = net
    with tf.variable_scope('Conv4'):
        net = resface_pre(net, 512 // 2, scope='Conv4_pre')  # 3x3
        net = slim.repeat(net, 1, resface_block, 512 // 2, scope='Conv4')  # 2
