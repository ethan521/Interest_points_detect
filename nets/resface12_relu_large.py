...
def resface_pre(lower_input,output_channels,stride=2,scope=None):
    net = slim.conv2d(lower_input, output_channels, stride=stride, scope=scope)
    return net

def resface12(images, keep_probability,
    ...
    with tf.variable_scope('Conv1'):
        net = resface_pre(images,64//2,stride=1,scope='Conv1_pre') # 48x48
        net = slim.repeat(net,1,resface_block,64//2,scope='Conv1')
    with tf.variable_scope('Conv2'):
        net = resface_pre(net,128//2,scope='Conv2_pre') # 24x24
        net = slim.repeat(net,1,resface_block,128//2,scope='Conv2')
    with tf.variable_scope('Conv3'):
        net = resface_pre(net,256//2,scope='Conv3_pre') # 12x12
        net = slim.repeat(net,1,resface_block,256//2,scope='Conv3')
    with tf.variable_scope('Conv4'):
        net = resface_pre(net,512//2,scope='Conv4_pre') # 6x6
        net = slim.repeat(net,1,resface_block,512//2,scope='Conv4')
...
def resface20(images, keep_probability,
    ...
    '''
    with tf.variable_scope('Conv1'):
        net = resface_pre(images,64,scope='Conv1_pre') # 56x56
        net = slim.repeat(net,1,resface_block,64,scope='Conv1')
    with tf.variable_scope('Conv2'):
        net = resface_pre(net,128,scope='Conv2_pre') # 28x28
        net = slim.repeat(net,2,resface_block,128,scope='Conv2')
    with tf.variable_scope('Conv3'):
        net = resface_pre(net,256,scope='Conv3_pre') # 14x14
        net = slim.repeat(net,4,resface_block,256,scope='Conv3')
    with tf.variable_scope('Conv4'):
        net = resface_pre(net,512,scope='Conv4_pre') # 7x7
    ...
    with tf.variable_scope('Resface'):
        with slim.arg_scope([slim.conv2d, slim.fully_connected], 
            ...
            with slim.arg_scope([slim.conv2d], kernel_size=3):
                return resface12(images=image_batch,
