...
def resface36(images, keep_probability, 
    ...
    with tf.variable_scope('Conv1'):
        ...
        net = slim.repeat(net,2,resface_block,64//2,scope='Conv_1')
    with tf.variable_scope('Conv2'):
        net = resface_pre(net,128//2,scope='Conv2_pre')
        net = slim.repeat(net,4,resface_block,128//2,scope='Conv_2')
    with tf.variable_scope('Conv3'):
        net = resface_pre(net,256//2,scope='Conv3_pre')
        net = slim.repeat(net,8,resface_block,256//2,scope='Conv_3')
    with tf.variable_scope('Conv4'):
    ...
    with tf.variable_scope('Resface'):
        with slim.arg_scope([slim.conv2d, slim.fully_connected], 
            ...
            with slim.arg_scope([slim.conv2d], kernel_size=3):
                return resface36(images=image_batch,
