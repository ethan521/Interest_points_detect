...
def resface20s(images, keep_probability,
    ...
    with tf.variable_scope('Conv1'):
        net = resface_pre(images,64//8*3,scope='Conv1_pre')
        net = slim.repeat(net,1,resface_block,64//8*3,scope='Conv1')
    with tf.variable_scope('Conv2'):
        net = resface_pre(net,128//8*3,scope='Conv2_pre')
        net = slim.repeat(net,2,resface_block,128//8*3,scope='Conv2')
    with tf.variable_scope('Conv3'):
        net = resface_pre(net,256//8*3,scope='Conv3_pre')
        net = slim.repeat(net,4,resface_block,256//8*3,scope='Conv3')
    with tf.variable_scope('Conv4'):
        net = resface_pre(net,512//8*3,scope='Conv4_pre')
        net = slim.repeat(net,1,resface_block,512//8*3,scope='Conv4')
